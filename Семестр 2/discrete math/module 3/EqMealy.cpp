...
struct Automat {
    ...
    bool operator==(Automat& b) {
        this->MinimizeAufenkampHohn();
        this->Canonize();

        b.MinimizeAufenkampHohn();
        b.Canonize();

        if (this->n != b.n || this->m != b.m || this->q0 != b.q0) {
            return false;
        }

        for (int i = 0; i < this->n; ++i) {
            for (int j = 0; j < this->m; ++j) {
                if (this->moves[i][j] != b.moves[i][j] || this->outs[i][j] != b.outs[i][j]) {
                    return false;
                }
            }
        }
